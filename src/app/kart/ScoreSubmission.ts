...
export default class ScoreSubmission {
	...
	private async submit(name: string, generation: number): Promise<void> {
		...
		try {
			const result: SubmittedScore | null = await submitScore(name, Math.round(this.timeMs));
			...
			// the stored score (a bodyless 2xx) - there's no id/editToken to rename
			// against in that case, so leave savedScore unset and treat the name as
			// fixed until a future request confirms one.
			this.savedScore = result
				? {id: result.id, editToken: result.editToken, name: result.name, rank: result.rank}
				: null;
	...
	private async refreshFinalBoard(generation: number): Promise<void> {
		...
		try {
			...
			// A missing `saved` means the server confirmed the save but didn't echo
			// back the stored score (a bodyless 2xx) - there's no id or rank to place
			// an own row with, so just highlight nothing outside the returned board.
			if (!saved) {
				this.hud.showLeaderboard(top, -1);
				return;
			}
	...
	private rename(name: string): void {
		...
		updateScoreName(saved.id, saved.editToken, name).then((result) => {
			...
			// A null result means the rename succeeded but the server didn't echo back
			// the stored score (a bodyless 2xx) - fall back to what was submitted and
			// keep the previously known rank rather than guessing at a new one.
			const savedName = result ? result.name : name;
			const savedRank = result ? result.rank : saved.rank;

			this.savedScore = {...saved, name: savedName, rank: savedRank};
			ScoreSubmission.savePlayerName(savedName);
			this.hud.setScoreSaved(savedName, savedRank);
			this.hud.updateOwnName(savedName);
